...
test('staff can create an activity', async ({ page }) => {
  ...
  await page.getByLabel(/imagem/i).fill('https://example.com/aula.jpg')
  await page.getByLabel(/vagas/i).fill('20')
  await page.getByRole('button', { name: /criar atividade/i }).click()
  await expect(page.getByRole('status')).toBeVisible({ timeout: 7000 })
})
...
test('staff can create a news item', async ({ page }) => {
  ...
  await page.getByRole('textbox', { name: 'Fonte' }).fill('https://example.com')
